...
path_to_raw_data = "../testData"
base_src_path = "../testData/original/"
base_dst_path = "../testData/procData/"
...
categories = [f.path for f in os.scandir(base_src_path) if f.is_dir()]


# Iterate through each subdirectory and category to copy and resize files
for category in categories:
    src_dir = category
    dst_dir = os.path.join(base_dst_path, "Normal" if category.lower().find("normal") >= 0 else "Pneumonia")
    print(category)
